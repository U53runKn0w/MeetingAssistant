...
@tool
def search_term_explanation(question: str) -> str:
    """
    【适用场景】当会议中出现AI需要联网查询的问题、专有名词、技术术语、行业术语时使用。
    【调用时机】遇到用户提问、陌生名词、缩写、专业术语需要联网查询解释时。
    【参数要求】question可以是任何问题或名词。例如："什么是RAG？"、"微服务架构的优缺点"、"AGI的发展现状"。
    【返回内容】返回基于搜索结果的详细回答。
    【工作原理】使用DuckDuckGo搜索引擎查询问题相关信息，并整合多个搜索结果生成答案。
    """
    try:
        from ddgs import DDGS

        # 使用DuckDuckGo搜索，添加重试机制
        ddgs = DDGS()
        search_results = []
        ...
        if not search_results:
            return f"未找到关于「{question}」的相关信息，可能是网络连接问题或该问题过于生僻。"
        ...
        # 生成详细的回答摘要
        if answers:
            combined_answer = "".join(answers[:3])  # 整合前三个结果
            return f"关于「{question}」的搜索结果：{combined_answer}"
        else:
            return f"找到相关结果但无法提取内容。"
